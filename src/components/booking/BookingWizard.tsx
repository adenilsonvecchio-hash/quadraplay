...
export const BookingWizard: React.FC<BookingWizardProps> = ({
  ...
  return <div className="space-y-3 pb-4">
    ...
    {step === 2 && <motion.section initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} className="space-y-3">
      <div className="qp-glass rounded-[26px] p-4"><div className="flex items-center justify-between mb-3"><div><h3 className="font-black text-[#101b3d]">Escolha a quadra</h3><p className="text-xs text-slate-500">{formatFriendlyDate(selectedDate)}</p></div><MapPin className="w-5 h-5 text-violet-500" /></div>{courts.length === 0 ? <div className="rounded-[18px] bg-amber-50 border border-amber-100 p-4 text-center"><p className="text-sm font-black text-amber-900">Nenhum espaço disponível</p><p className="text-xs text-amber-700 mt-1">A administração precisa liberar uma quadra para esta modalidade.</p></div> : <div className="grid grid-cols-3 gap-2">{courts.map(c => <button key={c.id} onClick={() => {setSelectedCourt(c);setSelectedSlot(null)}} className={`rounded-[18px] py-3 px-1 text-[10px] font-black ${selectedCourt?.id===c.id?'bg-gradient-to-br from-[#725cff] to-[#5038eb] text-white shadow-lg':'qp-button text-slate-600'}`}><div className="w-7 h-7 rounded-lg border-2 border-current mx-auto mb-1.5 flex items-center justify-center"><span className="w-4 h-px bg-current" /></div>{c.name}</button>)}</div>}</div>
