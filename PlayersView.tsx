...
export const PlayersView: React.FC<PlayersViewProps> = ({ onChallengePlayer }) => {
  ...
  const [selectedClass, setSelectedClass] = useState<TennisClass>(currentUser?.level || 'B');
  ...
  const isUserClass = currentUser?.level === selectedClass;
    ...
    <section className="qp-card rounded-[28px] p-3">
      ...
      <div className="space-y-2">{players.map(player => {
        ...
        return <div key={player.id} className={`qp-soft rounded-[19px] p-3 flex items-center gap-3 ${self ? 'ring-2 ring-violet-100' : ''}`}>
          ...
          <div className="min-w-0 flex-1"><p className="text-sm font-black truncate">{player.name}{self ? ' (Você)' : ''}</p><p className="text-[10px] text-slate-500">Classe {player.level}</p></div>
