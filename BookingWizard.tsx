...
export const BookingWizard: React.FC<BookingWizardProps> = ({
  ...
  const [courts, setCourts] = useState<Court[]>(() => usingSupabase ? [] : storageService.getCourts());
  const [selectedCourt, setSelectedCourt] = useState<Court | null>(() => usingSupabase ? null : storageService.getCourts().find(c => c.id === preselectedCourtId) || storageService.getCourts()[0] || null);
  ...
  return <div className="space-y-3 pb-4">
    <section className="qp-glass rounded-[26px] p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="text-center"><p className="text-[11px] font-bold text-amber-500 uppercase tracking-widest">Nova partida</p><h2 className="font-black text-[#101b3d]">{stepTitles[step-1]}</h2></div>
        <span className="text-[11px] font-black text-amber-700 bg-amber-50 px-2.5 py-1.5 rounded-full">{step}/4</span>
      </div>
      <div className="grid grid-cols-4 gap-1.5 mt-4">
        {stepTitles.map((title, i) => <div key={title} className="min-w-0"><div className={`h-1.5 rounded-full ${i+1 <= step ? 'bg-amber-500' : 'bg-slate-200'}`} /><p className={`mt-1 text-[8px] text-center truncate ${i+1===step?'text-amber-700 font-black':'text-slate-400'}`}>{title}</p></div>)}
      </div>
    </section>
    ...
    {step === 1 && <motion.section initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} className="qp-glass rounded-[26px] p-4">
      <div className="flex items-center gap-3 mb-4"><div className="w-11 h-11 rounded-2xl bg-amber-100 text-amber-500 flex items-center justify-center"><CalendarDays className="w-5 h-5" /></div><div><h3 className="font-black text-[#101b3d]">Selecione a data</h3><p className="text-xs text-slate-500">Somente hoje e datas futuras.</p></div></div>
      <div className="grid grid-cols-3 gap-2">
        {dates.map((date, i) => <button key={date} onClick={() => chooseDate(date)} className={`rounded-[18px] border p-3 text-center ${date===selectedDate?'bg-gradient-to-br from-[#ffca2d] to-[#e9a900] text-white border-white shadow-lg':'bg-white/65 border-white text-[#101b3d]'}`}><p className="text-[10px] font-bold opacity-70">{i===0?'HOJE':i===1?'AMANHÃ':formatFriendlyDate(date).split(',')[0].toUpperCase()}</p><p className="text-lg font-black mt-1">{date.slice(8,10)}</p><p className="text-[10px] opacity-70">{new Date(date+'T12:00:00').toLocaleDateString('pt-BR',{month:'short'})}</p></button>)}
      </div>
    </motion.section>}

    {step === 2 && <motion.section initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} className="space-y-3">
      <div className="qp-glass rounded-[26px] p-4"><div className="flex items-center justify-between mb-3"><div><h3 className="font-black text-[#101b3d]">Escolha a quadra</h3><p className="text-xs text-slate-500">{formatFriendlyDate(selectedDate)}</p></div><MapPin className="w-5 h-5 text-amber-500" /></div>{courts.length === 0 ? <div className="rounded-[18px] bg-amber-50 border border-amber-100 p-4 text-center"><p className="text-sm font-black text-amber-900">Nenhum espaço disponível</p><p className="text-xs text-amber-700 mt-1">A administração precisa liberar uma quadra para esta modalidade.</p></div> : <div className="grid grid-cols-3 gap-2">{courts.map(c => <button key={c.id} onClick={() => {setSelectedCourt(c);setSelectedSlot(null)}} className={`rounded-[18px] py-3 px-1 text-[10px] font-black ${selectedCourt?.id===c.id?'bg-gradient-to-br from-[#ffca2d] to-[#e9a900] text-white shadow-lg':'qp-button text-slate-600'}`}><div className="w-7 h-7 rounded-lg border-2 border-current mx-auto mb-1.5 flex items-center justify-center"><span className="w-4 h-px bg-current" /></div>{c.name}</button>)}</div>}</div>
      <div className="qp-glass rounded-[26px] p-3"><div className="flex items-center gap-2 px-1 pb-2"><Clock3 className="w-4 h-4 text-amber-500" /><h3 className="text-sm font-black text-[#101b3d]">Horários de {selectedCourt?.name || 'quadra'}</h3></div><div className="grid grid-cols-2 gap-2">{slots.map(slot => <button key={slot.startTime} disabled={!slot.available} onClick={() => chooseSlot(slot)} className={`rounded-[18px] p-3 text-left border ${slot.available?'bg-emerald-50/80 border-white text-emerald-800':'bg-slate-100/75 border-white text-slate-400'} disabled:cursor-not-allowed`}><div className="flex items-center justify-between"><span className="text-sm font-black">{slot.startTime}</span>{slot.available?<Check className="w-4 h-4"/>:<span className="text-[9px] font-bold">OCUPADO</span>}</div><p className="text-[10px] mt-1">até {slot.endTime}</p></button>)}</div></div>
    </motion.section>}

    {step === 3 && <motion.section initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} className="qp-glass rounded-[26px] p-4">
      <div className="flex items-center gap-3 mb-4"><div className="w-11 h-11 rounded-2xl bg-amber-100 text-amber-500 flex items-center justify-center"><UserRound className="w-5 h-5" /></div><div><h3 className="font-black text-[#101b3d]">Escolha o participante</h3><p className="text-xs text-slate-500">{isTennis ? `Jogadores da Classe ${currentUser.tennisClass}` : 'Todos os participantes aprovados'}</p></div></div>
      {loadingOpponents ? <div className="rounded-[18px] bg-white/60 border border-white p-5 text-center text-xs font-bold text-slate-500"><LoaderCircle className="w-5 h-5 mx-auto mb-2 animate-spin text-amber-500" />Carregando participantes...</div> : opponents.length === 0 ? <div className="rounded-[18px] bg-amber-50 border border-amber-100 p-4 text-center"><p className="text-sm font-black text-amber-900">Nenhum participante disponível</p><p className="text-xs text-amber-700 mt-1">{isTennis ? `É necessário outro jogador aprovado na Classe ${currentUser.tennisClass}.` : 'É necessário outro participante aprovado.'}</p><button type="button" onClick={() => window.location.reload()} className="mt-3 mx-auto px-4 py-2 rounded-xl bg-white text-amber-800 text-xs font-black flex items-center gap-1.5"><RefreshCw className="w-3.5 h-3.5" />Atualizar participantes</button></div> : <div className="space-y-2">{opponents.map(player => <button key={player.id} onClick={() => chooseOpponent(player)} className={`w-full rounded-[18px] p-3 flex items-center gap-3 text-left border ${selectedOpponent?.id===player.id?'bg-amber-50 border-amber-200':'bg-white/60 border-white'}`}><PlayerAvatar name={player.name} avatarUrl={player.avatarUrl} className="w-11 h-11 text-xs ring-2 ring-white shadow-sm" /><div className="min-w-0 flex-1"><p className="text-sm font-black text-[#101b3d] truncate">{player.name}</p>{isTennis && <p className="text-[10px] text-slate-500">Classe {player.tennisClass}</p>}</div><ChevronRight className="w-4 h-4 text-amber-500" /></button>)}</div>}
    </motion.section>}
    ...
    {step === 4 && <motion.section initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} className="space-y-3">
      ...
      <button disabled={saving} onClick={confirm} className="w-full h-14 rounded-[20px] bg-gradient-to-r from-[#ffca2d] to-[#e9a900] text-white font-black shadow-[0_12px_28px_rgba(190,145,0,0.28)] disabled:opacity-60">{saving?'Confirmando...':'Confirmar reserva'}</button>
    </motion.section>}
  </div>;
};
