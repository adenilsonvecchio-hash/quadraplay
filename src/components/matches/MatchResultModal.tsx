...
export const MatchResultModal: React.FC<Props> = ({ match, isTennis, busy, onClose, onSubmit }) => {
  ...
  return <div className="fixed inset-0 z-[80] bg-[#071633]/55 backdrop-blur-sm flex items-end sm:items-center justify-center p-3" role="dialog" aria-modal="true" aria-label="Informar resultado">
    <div className="w-full max-w-lg rounded-[28px] bg-white shadow-2xl overflow-hidden">
      ...
      <div className="p-5 max-h-[75vh] overflow-y-auto">
        ...
        <button type="button" disabled={busy} onClick={submit} className="mt-4 w-full rounded-2xl bg-[#101b3d] py-3.5 text-sm font-black text-white flex items-center justify-center gap-2 disabled:opacity-50"><CheckCircle2 className="w-5 h-5" />{busy ? 'Salvando...' : 'Enviar para confirmação'}</button>
      </div>
    </div>
  </div>;
};
