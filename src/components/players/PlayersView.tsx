...
export const PlayersView: React.FC<PlayersViewProps> = ({ onChallengePlayer }) => {
  ...
  return <div className="space-y-4 pb-8">
    ...
    <div className="relative"><Search className="w-4 h-4 text-slate-400 absolute left-4 top-1/2 -translate-y-1/2"/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar participante" className="w-full qp-soft rounded-[18px] pl-11 pr-4 py-3.5 text-sm outline-none focus:ring-2 focus:ring-amber-200"/></div>
    ...
    <section className="qp-card rounded-[28px] p-3">
      <div className="flex items-center justify-between px-1 pb-3"><div className="flex items-center gap-2"><Users className="w-4 h-4 text-amber-500"/><h3 className="text-sm font-black">{isTennis ? `Classe ${selectedClass}` : 'Participantes'}</h3></div><span className="text-[10px] text-slate-400 font-bold">{players.length} participantes</span></div>
      <div className="space-y-2">{players.map((player) => {
        const self = currentUser?.id === player.id;
        const canChallenge = isUserClass && !self;
        return <div key={player.id} className={`qp-soft rounded-[19px] p-3 flex items-center gap-3 ${self ? 'ring-2 ring-amber-100' : ''}`}>
          ...
          {canChallenge ? <button onClick={() => onChallengePlayer(player.id)} className="qp-primary rounded-[14px] px-3 py-2 text-xs font-black flex items-center gap-1"><Plus className="w-3.5 h-3.5"/>Agendar</button> : self ? <span className="text-[10px] font-black text-amber-500 bg-amber-50 px-2 py-1 rounded-lg">Seu perfil</span> : null}
        </div>;
      })}</div>
    </section>
  </div>;
